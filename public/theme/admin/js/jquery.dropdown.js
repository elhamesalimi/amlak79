...
;(function ($, window, document, undefined) {
    ...
    var mylist = $('.selcted-input').length;
    var i;

    // var array = [];

    for (i = 0; i < mylist; i++) {
        array.push(parseInt($('#selected' + i).val()));
    }

    // console.log(array);

    $('.delete-category').click(function () {
        $(this).parents("li.cat").remove();
        var id = $(this).parents("li.cat").find("input").val();
    ...
    dropdown.prototype = {
        ...
        _afterSelect: function (item, previous) {
            ...
            // $.map(self.inst.items, function(item, index) {
            //     if(item.selected){
            //         text=item.text;
            //     }
            // });
            // Event
            self.$elem.trigger(self.name + '.select', [item, previous, this]);
            // console.log(jQuery.inArray(item.value,array))


            if (jQuery.inArray(item.value, array) === -1) {
                array.push(item.value);
                $('#categories-wrraper').append(
                    ' <li class="cat" >' +
                    '    <span class="label label-success" style="margin-right: 1em ; margin-top: 2em; height: 50px"> <i\n' +
                    '                                                                class="fa fa-times delete-category mouse"></i> '+ item.text +' </span>\n' +
                    '                                                    <input class="selcted-input select" \n' +

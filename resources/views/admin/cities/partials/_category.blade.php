<li class=" @if(!$category->parent)h4 @endif">{{ $category->name }}
    {{--edit ajax--}}
    <a href="javascript:ajaxLoad('{{ action('Admin\VocabularyController@edit',['vocabulary'=>$category]) }}','create')"><span
    ...
    <div class="modal fade modal-sm " id="myMoldal{{$category->id}}" role="dialog">
        <div class="modal-content ">
            ...
            <div class="modal-footer">
                <div class="col-md-6">
                    <form action="{{ action('Admin\VocabularyController@destroy',['vocabulary'=>$category]) }}" id="frm" method="POST">
                        {{ csrf_field() }}
...
@if ($category->children->count() > 0)
    <ul>
        @foreach($category->children as $child)
            @include('admin.vocabularies.partials._category', ['category' => $child])
        @endforeach
    </ul>
@endif